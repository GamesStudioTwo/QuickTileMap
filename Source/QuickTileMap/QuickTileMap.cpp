...
void QuickTileMap::BuildTileMap( void )
{
   ...
   //auto group = map->getVectorOfChildrenByName({ "group" },true);  // true to reverse the order

    for( auto node  : tileMapNodes) {
      //  node->ShowAllStorage();
        /** Do something with an group **/
        if ( node->getName() == "group" ) {
           // tileMapNodes = noe->getVectorOfChildrenByName({ "objectgroup", "layer", "imagelayer" },true);  // true to reverse the order
         }
